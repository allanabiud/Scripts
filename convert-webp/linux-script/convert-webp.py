...
def getFileMimeType(file):
    command = shlex.split(f"file -b --mime-type {file}")
    try:
        rar_mime = ['application/vnd.rar', 'application/x-rar-compressed', 'application/x-rar', 'application/rar']
...
def createWorkDir(filename):
    cwd = pathlib.Path(os.getenv('C2W_PATH','D:\Books\Comics\TOOLS\Scripts\convert-webp'))
