...
print_welcome_message() {
	print_long_large_separator
	echo -e "${BLUE}                       MYLAR UTILITY SCRIPT ${NC}"
	print_long_large_separator

	print_message "This is a mylar utility script."
	print_message "It is able to:"
	echo -e "${YELLOW}- Connect to a Mylar3 instance${NC}"
	echo -e "${YELLOW}- Start, Stop and Restart the Mylar3 service${NC}"
	echo -e "${YELLOW}- Update the Mylar3 service${NC}"
	echo -e "${YELLOW}- Backup the Mylar3 database${NC}"
	echo -e "${YELLOW}- Trigger a library scan${NC}"
	echo -e "${YELLOW}- Check for and download missing issues${NC}"
	print_separator
	print_message "First, enter the following details to connect to your Mylar3 instance."
}

#######################################
####### GET MYLAR3 INSTANCE INFO ######
#######################################
get_remote_host() {
	while true; do
		print_message "Enter the remote host (${YELLOW}user@hostname${NC}):"
		read -rp " " remote_host
...
find_mylar_directories() {
	print_message "Searching for Mylar3 directories..."
	remote_exec "find / -type d \( -name '*mylar*' -o -name '*Mylar*' -o -name '*mylar3*' -o -name '*Mylar3*' \) 2>/dev/null"
}

get_mylar_directory() {
	while true; do
		print_message "Do you know the absolute path to your Mylar3 directory? (${YELLOW}y/n${NC})"
		read -p " " known_path
		if [[ $known_path =~ ^[yY]$ ]]; then
			print_message "Enter the Mylar3 directory (${YELLOW}absolute path${NC}):"
			...
			print_message "Let's try to find the Mylar3 directory..."
			potential_directories=$(find_mylar_directories)
			if [[ -z $potential_directories ]]; then
				print_error "Could not find Mylar3 directory."
				print_message "Please try to find the Mylar3 directory manually and enter it here."
			else
				print_message "Found potential Mylar3 directories:"
				echo "$potential_directories"
				print_message "Please choose one of these directories or enter a different path."
			fi
			read -p "Enter the Mylar3 directory (${YELLOW}absolute path${NC}): " mylar_directory
			if validate_directory "$mylar_directory"; then
				print_success "Mylar3 directory set to ${YELLOW}$mylar_directory${NC}"
				break
			else
...
get_backup_directory() {
	while true; do
		print_message "Enter the backup directory (${YELLOW}absolute path${NC}):"
...
get_mylar_port() {
	while true; do
		print_message "Enter the Mylar3 port (${YELLOW}default: 8090${NC}):"
		read -rp " " mylar_port
